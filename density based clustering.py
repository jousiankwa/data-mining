...
#爬坡
#根据原始论文，需要计算收敛时的最后两个距离和作为密度吸引子的半径，这里计算了3个
...
def _step(x_l0, X, W=None, h=0.1):
    n = X.shape[0]
    d = X.shape[1]
    superweight = 0.  #superweight是计算X(t+1)时的分母
    x_l1 = np.zeros((1, d))
    if W is None:
        W = np.ones((n, 1))
    else:
        W = W
    for j in range(n):
        kernel = kernelize(x_l0, X[j], h, d)    #x_l0为X(t)
        kernel = kernel * W[j] / (h ** d)       #W[j]为权重，但是实际上都为1
        superweight = superweight + kernel
        x_l1 = x_l1 + (kernel * X[j])   #x_l1为X(t+1)
...
#BaseEstimator为参数h，ClusterMixin为参数eps（ε）
class DENCLUE(BaseEstimator, ClusterMixin):
    def __init__(self, h=None, eps=1e-8, min_density=0.2, metric="euclidean"):
        self.h = h
        self.eps = eps
        ...
        for j1 in range(self.n_samples):
            for j2 in (x for x in range(self.n_samples) if x != j1):
                if g_clusters.has_edge(j1, j2):
                    continue
                #对于每个计算所得到的密度吸引子，如果半径和小于他们的距离，说明它们所吸引的点有共同的真正的密度吸引子
                diff = np.linalg.norm(g_clusters.nodes[j1]["dict"]["attractor"] - g_clusters.nodes[j2]["dict"]["attractor"])
                if diff <= (g_clusters.nodes[j1]["dict"]["radius"] + g_clusters.nodes[j2]["dict"]["radius"]):
                    #加边，链接，下一步方便计算最大连通子图
...
for i in d.clust_info_.keys():
    ...
    aa=list(d.clust_info_[i]["centroid"])
    plt.scatter(aa[0],aa[1],c='k',marker='o')
plt.show()
